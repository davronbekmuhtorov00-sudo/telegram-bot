...
ZAXIRA_GAPLAR = [
    ...
    "Севги бошланади осон, лекин сақлаш қийин.",
    "Вақт ўтади, лекин қилган яхшилик қолади.",
    "Пул топиш осон, лекин уни тўғри сарфлаш қийин.",
...
def ai_dan_gap_ol(mavzu):
    prompt = (
        ...
        "2. Фақат 1 та жумла\n"
        "3. 8 дан 12 гача сўз — на кам, на кўп\n"
...
def generate_post() -> str:
    ...
    for urinish in range(5):
        ...
        if lotin_harfmi(gap):
            logger.warning(f"Lotin harf! Qayta: {gap[:40]}")
            continue

        if taqiqlangan_boshlashmi(gap):
            logger.warning(f"Taqiqlangan boshlash! Qayta: {gap[:40]}")
            continue

        if not soz_soni_togriymi(gap):
            logger.warning(f"Soz soni noto'g'ri! Qayta: {gap[:40]}")
            continue

        logger.info(f"Gap tasdiqlandi: {gap[:60]}")
        return gap

    logger.warning("5 urinishda ham xato, zaxira gap ishlatildi")
